...
int main(int argc, char** argv) {
    if (argc != 8) { fprintf(stderr, "usage: %s a.param a.bin b.param b.bin filelist.txt W H\n", argv[0]); return 1; }
    ...
    while (std::getline(lf, path)) {
        ...
        double mx = 0; int agree = 0;
        for (size_t i = 0; i < oa.size(); i++) mx = std::max(mx, (double)fabs(oa[i] - ob[i]));
