...
> How the `app/src/main/assets/ncnn/*.param/*.bin` models (det `960×960`, rec `48×64/128/256/480`) were produced from the canonical Paddle `model.safetensors`.
...
Archived per user request: onnx/pdiparams/safetensors moved from `app/src/main/assets/PP-OCRv6_*` → `models/archive/PP-OCRv6_*` so the APK ships only `ncnn/` (`10.56 MB` per rec bucket, `det 4.8 MB`) + `vocab.json`.
...
- **Outputs**: `app/src/main/assets/ncnn/rec_w{64,128,256,480}.param/.bin` (Git LFS, `app/src/main/assets/ncnn/*.bin`).
...
- **Outputs**: `app/src/main/assets/ncnn/det.param` (`19 KB`) / `det.bin` (`4.8 MB`), `242` nodes, input `in0` `[1,3,960,960]` `NCHW` ImageNet `(0.485/0.456/0.406 / 0.229/0.224/0.225)`, output prob map `1×960×960` `sigmoid`.
...
python tools/convert_safetensors_to_onnx.py \
  ...
  --widths 64,128,256,480 --height 48 --out app/src/main/assets/ncnn
...
# copy to app/src/main/assets/ncnn/ (LFS)
...
app/src/main/assets/
  ncnn/ — det.param/bin, rec_w{64,128,256,480}.param/bin (LFS, shipped)
  PP-OCRv6_small_rec_onnx/vocab.json — 18710, shipped (only vocab remains from PP-OCRv6 onnx dirs)
```
